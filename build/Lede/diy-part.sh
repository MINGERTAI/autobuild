...
# 后台IP设置
export Ipv4_ipaddr="192.168.0.77"            # 修改openwrt后台地址(填0为关闭)
export Netmask_netm="255.255.255.0"         # IPv4 子网掩码（默认：255.255.255.0）(填0为不作修改)
export Op_name="OpenWrt-QTM"                # 修改主机名称为OpenWrt-123(填0为不作修改)
...
# 旁路由选项
export Gateway_Settings="192.168.0.77"                 # 旁路由设置 IPv4 网关(填入您的网关IP为启用)(填0为不作修改)
export DNS_Settings="192.168.1.1 192.168.0.2"                     # 旁路由设置 DNS(填入DNS，多个DNS要用空格分开)(填0为不作修改)
...
# 更换固件内核
export Replace_Kernel="6.1.60"                    # 更换内核版本,在对应源码的[target/linux/架构]查看patches-x.x,看看x.x有啥就有啥内核了(填入内核x.x版本号,填0为不作修改)
